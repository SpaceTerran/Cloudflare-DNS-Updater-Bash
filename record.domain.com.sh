...
# Get the current content of the A record (only the IP address)
record_content=$(echo "$response" | jq -r '.result[] | select(.name=="'"$RECORD_NAME"'" and .type=="A") | .content')
echo "Current Record IP: $record_content"
